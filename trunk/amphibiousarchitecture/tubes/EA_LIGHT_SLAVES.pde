/* EA_LIGHT_SLAVES.pde
 * Arduino 2009 using Arduino 0016 over RS485
 * light dances, frame checking on RS485 messages
 * dar/theliving/2009-08-06
 * kcw/theliving/2009-09-13
 * jcl/xclinic/2009-09-13
 */
...
void loop () {
  ...
  if (inString.charAt(0) == '#' && inString.length() == 7 && inString.charAt(6) == '!'){
    //strncpy (IDStr,inString,2);
    IDStr.append(inString.charAt(1)); 
    IDStr.append(inString.charAt(2)); 
    dance = inString.charAt(3);
    dissox = inString.charAt(4);
    event = inString.charAt(5);
    //Serial.write(inString);
    //Serial.println(IDStr);
  }

/*
  if (inString.charAt(2) == 't'){ // test sequence
    setColor(ledPinTop, RED); 
    setColor(ledPinBot, RED); 
    delay (1000);
    setColor(ledPinTop, BLUE); 
    setColor(ledPinBot, BLUE); 
    delay (1000);
    setColor(ledPinTop, GREEN); 
    setColor(ledPinBot, GREEN); 
    delay (1000);
    setColor(ledPinTop, WHITE); 
    setColor(ledPinBot, WHITE); 
    delay (1000);
  }
  */
  //else {
