 ...
 * manage sensor and sms udp packets, send light behavior packets
...
void draw() {
  ...
  if (dissox > 100 ){  //////FLIP NUMBER FOR DISSOX *****************************************************
    oxygen = 1;
  }
  if (dissox <= 100){
    oxygen = 0;
...
// FISH SWIM BY
      ...
      // long UDP
      String[] strsend = { 
       "#action=f1&site=2&id=",levelTop[i][j].id,"&fishon=", str(c), "&dissox=",str(o),"&event=",str(e), "&!" }; 
...
// DISSOX AND SMS EVENT
  if (trackOxygen != oxygen || trackSMS != event && countSmsOx == 0){
    /*
    // long UDP
    String[] strsendAll = { 
     "#action=f1&site=2&id=","22","fishon=", "9", "&dissox=",str(o),"&event=",str(e), "&!"  }; 
    */
